...
pub fn py_predict_im(
    ...
    for ((orig_idx, _), updated) in indexed_feats.iter().zip(feats.iter()) {
        ...
        psm_borrow.inner.sage_feature.predicted_ims   = updated.predicted_ims;
        psm_borrow.inner.sage_feature.delta_ims_model = updated.delta_ims_model;
    }
